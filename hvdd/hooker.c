    ...
    while (Module32Next(ModuleSnapshot, &me32))
    {
        if(_wcsicmp(me32.szModule, L"dbgeng.dll") == 0)
        {
            ...
            if (NtStatus != STATUS_SUCCESS) {
                Red("HookKd NtAllocateVirtualMemory failed");
                goto Exit;
            }
			...
			if (Status != TRUE) goto Exit;
			 wprintf(L"MAPVIEWOFFILE_OFFSET: %08X-%08X\n", (ULONG)MAPVIEWOFFILE_OFFSET, (ULONG)MAPVIEWOFFILE_OFFSET + 0xB00);
			Status = WriteProcessMemory(ProcessHandle, MAPVIEWOFFILE_OFFSET, &MyMapViewOfFile, 0xB00, &size);
			if (Status != TRUE) goto Exit;
			...
			if (Status != TRUE) goto Exit;
			 wprintf(L"VIRTUALPROTECT_OFFSET: %08X-%08X\n", (ULONG)VIRTUALPROTECT_OFFSET, (ULONG)VIRTUALPROTECT_OFFSET + 0x900);
...
FT = (PFUNCTION_TABLE)TABLE_OFFSET;
    ...
    if (hFileMappingObject == (HANDLE)0x1337)
    {
        ...
        //KDBGPa.LowPart = FT->KdDebuggerDataBlockPa.LowPart & 0xFFFFFFFFFFFF0000;
        j = FT->KdDebuggerDataBlockPa.LowPart & 0xFFFF;
        if ((KDBGPa.LowPart == Offset.LowPart) & (KDBGPa.HighPart == Offset.HighPart))
        {
            lol = (ULONG)(KDBGPa.HighPart);
            Str1[7] = Alpha[(UCHAR)(lol & 0xF)];
            Str1[6] = Alpha[(UCHAR)((lol >> 4) & 0xF)];
            Str1[5] = Alpha[(UCHAR)((lol >> 8) & 0xF)];
            Str1[4] = Alpha[(UCHAR)((lol >> 12) & 0xF)];
            ...
            {
                Buffer[i+j+0x2000] = FT->KdDebuggerDataBlockBlock[i];
                //Buffer[i+j+0x2000] = 0xCC;
            }
        }
#endif
